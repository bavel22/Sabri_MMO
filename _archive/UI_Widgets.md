...
| WBP_GameHud | Main HUD with health/mana/target | AC_HUDManager |
| WBP_ChatBox | In-game chat panel | AC_HUDManager |
| WBP_InventoryWindow | Inventory grid | AC_HUDManager |
| WBP_InventorySlot | Single inventory slot | WBP_InventoryWindow |
| WBP_StatAllocation | Stat point allocation | AC_HUDManager |
| WBP_DeathOverlay | Death screen | AC_HUDManager |
| WBP_LootPopup | Loot notification popup | AC_HUDManager |
| WBP_LootItemLine | Individual loot item display | WBP_LootPopup |
| WBP_DamageNumber | Floating damage text | AC_HUDManager |
| WBP_TargetHealthBar | Health bar above target | BP_OtherPlayerCharacter / BP_EnemyCharacter (Widget Component) |
| WBP_PlayerNameTag | Name tag above character | BP_OtherPlayerCharacter / BP_EnemyCharacter (Widget Component) |
...
### Purpose
Loot notification popup that displays items received from enemy kills. Managed by AC_HUDManager with auto-fade functionality.

### Hierarchy
```
Canvas Panel
└── Border (semi-transparent black background)
    └── Vertical Box
        ├── Text Block: EnemyNameText ("Loot from EnemyName")
        ├── Scroll Box: ItemListScrollBox
        │   └── (WBP_LootItemLine instances)
        └── Spacer (10px)
```
...
|------|------|---------|---------|
| EnemyName | String | "" | Enemy name for header display |
| FadeStartTime | Float | 0.0 | When popup started fading |
| FadeDuration | Float | 3.0 | Seconds before complete fade |
| CurrentOpacity | Float | 1.0 | Current opacity for fade effect |

### Functions

**ShowLoot(EnemyName, ItemsArray):**
```
Set EnemyNameText = ("Loot from " + EnemyName)
Set FadeStartTime = Get Game Time in Seconds
Set CurrentOpacity = 1.0
Set Visibility = Visible
    ↓
Clear Children (ItemListScrollBox)
    ↓
For Each Loop (ItemsArray)
    ├─ Create Widget: WBP_LootItemLine
    ├─ Set ItemText = Array Element
    ├─ Set ItemIconPath = "default_item"
    └─ Add Child to ItemListScrollBox
```

**Event Tick (Auto-fade logic):**
```
Event Tick (Delta Seconds)
    ↓
Branch: CurrentOpacity > 0?
    ├─ TRUE:
    │   ├─ CurrentTime = Get Game Time in Seconds
    │   ├─ ElapsedTime = CurrentTime - FadeStartTime
    │   ├─ Branch: ElapsedTime >= FadeDuration?
    │   │   ├─ TRUE:
    │   │   │   ├─ Set CurrentOpacity = 0.0
    │   │   │   └─ Remove from Parent
    │   │   └─ FALSE:
    │   │       └─ Set CurrentOpacity = 1.0 - (ElapsedTime / FadeDuration)
    │   └─ Set Render Opacity = CurrentOpacity
    └─ FALSE: (do nothing)
```

### Creation
Created by **AC_HUDManager.ShowLootPopup()** when `loot:drop` event is received. Multiple popups can be displayed simultaneously (max 5 concurrent).
...
### Purpose
Child widget that displays a single loot item with icon and text. Used by WBP_LootPopup.

### Hierarchy
```
Canvas Panel
└── Horizontal Box
    ├── Image: ItemIcon (32x32)
    └── Text Block: ItemTextBlock
```
...
|------|------|------------|---------|
| ItemText | String | Instance Editable: ✓, Blueprint Read Write: ✓ | Text to display (e.g., "2x Gloopy Residue") |
| ItemIconPath | String | Instance Editable: ✓, Blueprint Read Write: ✓ | Texture path for item icon |

### Functions

**Event Construct:**
```
Load Texture2D (ItemIconPath) → Make Brush → Set Brush from Texture (ItemIcon)
```

**Get_ItemText_Text (Binding):**
```
Return ItemText (variable)
```

### Usage
Created dynamically by WBP_LootPopup.ShowLoot() for each item in the loot array.
...
```
BP_SocketManager
    ├── Forwards events to → AC_HUDManager
    │   ├── combat:health_update → UpdatePlayerHealth/UpdatePlayerMana
    │   ├── combat:damage → ShowDamageNumber
    │   ├── combat:death → ShowDeathOverlay
    │   ├── loot:drop → ShowLootPopup
    │   ├── inventory:data → RefreshInventory
    │   ├── player:stats → UpdateStats
    │   └── chat:receive → ChatWidgetRef.AddChatMessage
    │
    └── (No longer creates widgets directly)

AC_HUDManager (Component on BP_MMOCharacter)
    ├── Creates → WBP_GameHud (InitializeHUD)
    ├── Creates → WBP_ChatBox (InitializeHUD)
    ├── Creates/Toggles → WBP_InventoryWindow (ToggleInventory)
    ├── Creates/Toggles → WBP_StatAllocation (ToggleStats)
    ├── Creates → WBP_DeathOverlay (ShowDeathOverlay)
    ├── Creates → WBP_LootPopup (ShowLootPopup)
    ├── Creates → WBP_DamageNumber (ShowDamageNumber)
    ├── Updates → WBP_GameHud.UpdateHealth/UpdateMana/UpdateTargetHealth
    └── Tracks → bIsInventoryOpen, bIsStatWindowOpen

WBP_InventoryWindow
    ├── Contains → SlotContainer (Wrap Box)
    ├── Calls → PopulateSlots (parses JSON, creates WBP_InventorySlot instances)
    ├── Handles → OnSlotAction (from child slots) → forwards to AC_HUDManager
    └── References → SocketManagerRef for emit

WBP_InventorySlot (Child Widget)
    ├── Variables (Expose on Spawn): InventoryID, bIsEquipped, ItemName, ItemType
    ├── Has → OnSlotAction Event Dispatcher (InventoryID, bWantToEquip)
    └── OnClicked → NOT bIsEquipped → Call OnSlotAction
...
BP_MMOCharacter
    ├── Has → AC_HUDManager component
    └── Calls → HUDManager.InitializeHUD() in BeginPlay
```
