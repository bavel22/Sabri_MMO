...
│ AC_HUDManager    │    data/used/   │                              │     │ character_inventory  │
│  - Loot Popups   │    equipped/    │ Item Helpers                 │     │  - per-character     │
│                  │    dropped/     │  - loadItemDefinitions()     │ ──► │    item ownership    │
│                  │    error        │  - addItemToInventory()      │     │  - quantities        │
│                  │                 │  - removeItemFromInventory() │     │  - equipped state    │
│ WBP_LootPopup    │ ◄── loot:drop   │  - getPlayerInventory()      │     └─────────────────────┘
│  - Auto-fade     │                 │  - rollEnemyDrops()          │
└──────────────────┘                 └──────────────────────────────┘
```
...
### AC_HUDManager Integration
- Component added to BP_MMOCharacter that manages all HUD elements
- Variables: `ActiveLootPopups` (Array), `MaxConcurrentPopups` (Integer = 5)
- Functions: `ShowLootPopup(EnemyName, ItemsArray)`, `CleanupLootPopups()`
- Tick integration for popup lifecycle management
- Forwards loot events from BP_SocketManager to WBP_LootPopup instances

### WBP_LootPopup
- Parent widget for loot notifications with auto-fade (3 seconds)
- Variables: `EnemyName`, `FadeStartTime`, `FadeDuration`, `CurrentOpacity`
- Functions: `ShowLoot(EnemyName, ItemsArray)`
- Event Tick handles opacity fade and self-destruction
- Creates WBP_LootItemLine children for each dropped item

### WBP_LootItemLine
- Child widget displaying individual loot item (icon + text)
- Variables: `ItemText` (String), `ItemIconPath` (String)
- Text binding to ItemText variable
- Icon loading from texture path in Event Construct

### BP_SocketManager Updates
- Added `HUDManager` variable (AC_HUDManager reference)
- Event BeginPlay gets HUDManager component from BP_MMOCharacter
- `OnLootDrop` handler forwards to `HUDManager.ShowLootPopup()`
- Removed old direct WBP_LootPopup creation code
...
- `loot:drop` → OnLootDrop → forwards to AC_HUDManager.ShowLootPopup

### AC_HUDManager Event Flow
- Receives loot:drop events from BP_SocketManager
- Creates WBP_LootPopup instances (max 5 concurrent)
- Manages popup lifecycle and cleanup via Tick
- Handles multiple simultaneous loot notifications
...
| Pattern | How Applied |
|---------|-------------|
| Component-Based | AC_HUDManager handles all HUD management separate from BP_SocketManager |
| Widget Composition | WBP_LootPopup composed of WBP_LootItemLine children |
| Event-Driven | loot:drop event triggers popup creation, no polling |
| Manager Pattern | AC_HUDManager centralizes all HUD element lifecycle |
| Single Responsibility | Each widget has one purpose: display, timing, or item representation |

---

**Last Updated**: 2026-02-16
**Version**: 1.3
**Status**: Loot system moved to AC_HUDManager with centralized popup management
