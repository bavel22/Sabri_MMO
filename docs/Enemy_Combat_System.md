...
- **Variables**: EnemyId (Int), EnemyName (String), EnemyLevel (Int), TargetPosition (Vector), Health (Float), MaxHealth (Float), IsDead (Bool), IsActiveTarget (Bool)
- **Event Tick**: Interpolate toward TargetPosition (same pattern as BP_OtherPlayerCharacter)
- **On Death**: Set IsDead=true, hide widgets, disable collision (CapsuleComponent + Mesh → Set Collision Enabled: No Collision)
- **On Respawn**: Set IsDead=false, show widgets, re-enable collision (Query and Physics), teleport to spawn position
...
**Version**: 1.0
**Status**: Complete (Server-side + Blueprint instructions provided)
