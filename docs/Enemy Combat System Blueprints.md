...
| `IsDead` | Boolean | false | Death state |
| `IsActiveTarget` | Boolean | false | Currently selected by local player |
...
## Step 1.7: Event Tick — Movement Toward TargetPosition

Same movement logic as BP_OtherPlayerCharacter:
...
**Node-by-node:**
1. From `Event Tick` exec → drag → **Branch** → Condition: drag `IsDead` variable
2. **FALSE** exec → **Get Actor Location** → promote to local var or just use the pin
...
Branch: IsTargetingEnemy AND TargetEnemyId > 0?
    ...
    │       → Branch: WasFound?
    │           └─ TRUE:
    │               EnemyActor → Set IsActiveTarget = false
