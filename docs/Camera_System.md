...
Event Tick
    ↓
Branch (IsRotatingCamera?)
    └─ TRUE:
        Get SpringArm
            ↓
        Get Relative Rotation → Break Rotator
            ├─ Pitch: (keep current - connect directly to Make Rotator)
            └─ Yaw: CurrentYaw + (LookInput.X * RotationSpeed)
                ↓
        Make Rotator (Roll=0, Pitch, Yaw)
            ↓
        Set Relative Rotation (SpringArm)
```

**Key Points:**
- Only Yaw changes (horizontal rotation)
- Pitch stays constant (maintains top-down angle)
- RotationSpeed multiplier (e.g., 2.0) controls sensitivity
...
Get Target Arm Length
    ↓
Float - (ActionValue * ZoomSpeed)
    ↓
Clamp (Min: 200.0, Max: 1500.0)
    ↓
Set Target Arm Length (SpringArm)
```
...
**Cause**: Missing IsRotatingCamera boolean variable
**Solution**: Create boolean variable, set on Triggered/Completed
