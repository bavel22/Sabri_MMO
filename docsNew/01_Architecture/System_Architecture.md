...
> **Navigation**: [Documentation Index](DocsNewINDEX.md) | [Multiplayer_Architecture](Multiplayer_Architecture.md) | [Database_Architecture](Database_Architecture.md) | [Networking_Protocol](../04_Integration/Networking_Protocol.md)
...
| `itemDefinitions` | `Map<itemId, ItemDef>` | Cached item DB rows (6,169 rAthena items) |
...
- **`UMultiplayerEventSubsystem`** — UWorldSubsystem providing `BlueprintCallable` emit methods: `EmitCombatAttack`, `EmitStopAttack`, `RequestRespawn`, `EmitChatMessage`. All inbound event bridges have been removed (Phases A-F); all inbound events are handled by dedicated C++ subsystems.
...
- **`CombatActionSubsystem`** — 10 combat event handlers, target frame (Z=9), death overlay (Z=40)
...
- **`ChatSubsystem`** — Chat window with 3 tabs: All/System/Combat (SChatWidget, Z=13)
...
- **`HotbarSubsystem`** — 4x9 hotbar, F5 cycle (SHotbarRowWidget x4, Z=16)
...
- **`ItemInspectSubsystem`** — Right-click inspect popup (SItemInspectWidget, Z=22)
...
- **`OtherPlayerSubsystem`** — Other player entity registry (TMap<int32, FPlayerEntry>), 2 event handlers, spawns BP_OtherPlayerCharacter
- **`TargetingSubsystem`** — 30Hz hover trace, cursor switching, hover indicators
- **`PlayerInputSubsystem`** — Click-to-move, click-to-attack, click-to-interact
- **`CameraSubsystem`** — Right-click yaw rotation, scroll zoom, fixed pitch
...
- **`BP_OtherPlayerCharacter`** — Remote player actor with interpolation (spawned by UOtherPlayerSubsystem)
- **`BP_EnemyCharacter`** — Client-side enemy actor (spawned by UEnemySubsystem)

> **Deleted Blueprints (historical):** BP_SocketManager (fully dead code, all handler functions deleted), BP_OtherPlayerManager (replaced by UOtherPlayerSubsystem), BP_EnemyManager (replaced by UEnemySubsystem), AC_HUDManager (replaced by 25 C++ Slate subsystems), AC_TargetingSystem (replaced by UTargetingSubsystem), AC_CameraController (replaced by UCameraSubsystem), BP_GameFlow (replaced by LoginFlowSubsystem). All 23 WBP_ UMG widgets have been replaced by pure C++ Slate widgets.
...
`UMultiplayerEventSubsystem` is a UWorldSubsystem that provides `BlueprintCallable` emit methods for Blueprints that need to send events:
- `EmitCombatAttack(TargetId, IsEnemy)`
- `EmitStopAttack()`
- `RequestRespawn()`
- `EmitChatMessage(Channel, Message)`

> **Historical note:** This subsystem originally bridged ~31 inbound socket events to `BP_SocketManager` handler functions via `FindFunction()`/`ProcessEvent()`. All bridges were removed in Phases A-F (2026-03-14). All inbound events are now handled by dedicated C++ subsystems (CombatActionSubsystem, EnemySubsystem, OtherPlayerSubsystem, InventorySubsystem, HotbarSubsystem, ShopSubsystem, ChatSubsystem, etc.).
...
All Blueprint emit calls go through the GameInstance, which forwards to the persistent `FSocketIONative`. BP_SocketManager is fully dead code -- all handler functions were deleted and the actor was removed from levels in Phase 6.
...
| **Manager** | `UEnemySubsystem` (enemy registry), `UOtherPlayerSubsystem` (player registry) |
...
| **WorldSubsystem** | 33 C++ UWorldSubsystems — each owns one domain (targeting, combat, UI panels, entity management, party, vending, pets, etc.) |
...
**Last Updated**: 2026-03-14 (Updated: removed stale BP references, corrected subsystem count to 25, corrected MultiplayerEventSubsystem to emit-only)
