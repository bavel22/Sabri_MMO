...
Input Action IA_CameraRotate (Started)
    ↓
Get CameraController → SetRotating(true)   [Phase 4: routed to AC_CameraController]

Input Action IA_CameraRotate (Completed)
    ↓
Get CameraController → SetRotating(false)  [Phase 4: routed to AC_CameraController]
```
