"""Imports the female knuckle weapon overlay atlases into UE5.

Usage (in UE5 editor Python console):
    py "C:/Sabri_MMO/client/SabriMMO/Scripts/Environment/import_weapon_knuckle_female.py"

Imports 17 weapon_12_*.png atlases. Set DELETE_FIRST=True to drop stale .uasset.
"""
import unreal
import os
import glob
...
DELETE_FIRST = True  # safe re-runs: drop old .uasset before importing fresh PNGs
...
unreal.log(f"[knuckle/female] Found {len(pngs)} PNG atlases to import")
...
unreal.log(f"[knuckle/female] Imported {len(tasks)}, configured {configured}")
