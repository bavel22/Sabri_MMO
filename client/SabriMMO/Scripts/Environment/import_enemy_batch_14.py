"""Batch 14 of 14 - imports 2 enemy sprite folders into UE5 (final batch).

Usage (in UE5 editor Python console):
    py "C:/Sabri_MMO/client/SabriMMO/Scripts/Environment/import_enemy_batch_14.py"
"""
import unreal
import os
import glob
...
total_imported = 0
total_configured = 0
unreal.log(f"[Batch 14/14] Importing {len(FOLDERS)} enemy sprite folder...")
for name in FOLDERS:
...
unreal.log(f"\n{'=' * 50}")
unreal.log(f"  [Batch 14/14] Total imported:   {total_imported}")
unreal.log(f"  [Batch 14/14] Total configured: {total_configured}")
unreal.log(f"{'=' * 50}")
