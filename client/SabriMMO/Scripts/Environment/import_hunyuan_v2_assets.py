# import_hunyuan_v2_assets.py
# Bulk import v2 Hunyuan GLBs from C:/Sabri_MMO/3d art/Final_v2/.
# Same flat layout as v1 import, with one addition: sets Collision Complexity
# = "Use Complex Collision As Simple" on each StaticMesh DURING import, so
# placed instances pick up the right collision from the start (no post-import
# fix required).
#
# Final layout (same as v1 — v2 assets coexist in the same folders):
#   /Game/SabriMMO/Environment/Hunyuan/
#     <category>/
#       <asset>_v2                      <- static mesh (no prefix; matches GLB name)
#       <asset>_v2_<variant>            <- variant static mesh
#       Materials/
#         M_<asset>_v2                  <- auto-imported material, renamed
#         M_<asset>_v2_<variant>
#       Textures/
#         T_<asset>_v2_*                <- moved + prefixed
#
# Run from UE5 Output Log (Python mode):
#   py "C:/Sabri_MMO/client/SabriMMO/Scripts/Environment/import_hunyuan_v2_assets.py"
...
# CONFIG
# ============================================================================
SOURCE_ROOT = "C:/Sabri_MMO/3d art/Final_v2"
...
def main():
    unreal.log("=" * 70)
    unreal.log("HUNYUAN V2 ASSET IMPORT (with collision = complex as simple)")
    ...
    for cat in CATEGORIES:
        ...
        for g in sorted(glob.glob(f"{cat_dir}/*.glb")):
            files.append((cat, g))
    unreal.log(f"\nFound {len(files)} v2 GLBs to import")
