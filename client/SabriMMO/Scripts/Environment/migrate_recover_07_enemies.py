"""RECOVERY part 7 of 7: re-import 5 test enemies from PNG.

Covers: poring, fabre, pupa, orc_warrior, zerom.

Run from UE5 Python console:
    py "C:/Sabri_MMO/client/SabriMMO/Scripts/Environment/migrate_recover_07_enemies.py"

After it finishes, wait for "Compiling Textures: 0", then restart the editor.
All 7 recovery scripts are now done.
"""
import unreal
import os
import glob
...
def main():
    ...
    for enemy in ENEMIES:
        ...
        if not pngs:
            unreal.log_warning(f"  no PNGs at {disk}")
            continue
        for png in pngs:
    ...
    unreal.log(f"")
    unreal.log(f"ALL RECOVERY PARTS COMPLETE.")
    unreal.log(f"Wait for 'Compiling Textures: 0', then restart the editor.")
    unreal.log(f"Atlases should now look exactly as they did before any migration.")
