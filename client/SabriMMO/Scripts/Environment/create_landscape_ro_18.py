# create_landscape_ro_18.py
# Run in UE5 Editor via: Edit > Execute Python Script
#
# Creates M_Landscape_RO_18 — extends v17 with paint-driven PAVEMENT layers.
#
# Existing v17 features (all preserved):
#   - 4-slot auto-blend: GrassWarm, GrassCool, Dirt, Rock
#   - Slope-based grass→rock blending (45° cutoff, configurable)
#   - Noise-driven grass variant splotches
#   - Macro variation (anti-tile-repetition)
#   - Noise-driven dirt patches
#   - World-space UVs with anti-seam UV distortion
#   - Normal map blending + AO
#   - 23 MI-overridable parameters
#   - Paint-driven Grass Output (sprite scatter via GrassDense/FlowerPatch/Debris)
#
# v18 NEW additions:
#   - 3 pavement texture slots: BrickTexture, CobbleTexture, PebbleTexture
#   - 3 paint layers (Brick/Cobble/Pebble) drive base-color blending
#   - Paint layers override the grass macro output where painted
#   - Total paint weight clamped 0..1 — unpainted areas fall through to v17 baseline
#   - Useful for capital town zones (Pryth-first, reusable for Geffen/Payon/Morroc)
#
# Reference: docsNew/05_Development/Ground_Texture_System_Research.md
#            docsNew/05_Development/Pryth_Build_Plan.md §5.1
...
MAT_DESCRIPTION = "v18 — Paint-driven PAVEMENT layers (Brick/Cobble/Pebble). LandscapeLayerSample drives base color blending so painted areas show different pavement textures. Includes all v17 features (4-slot grass+dirt+rock auto-blend, slope rock, normals, AO, 23 params) PLUS 3 new pavement texture slots and 3 paint-layer-driven blends. Designed for capital town zones (Pryth-first, reusable for Geffen/Payon/Morroc)."
...
# v18 NEW: pavement tile sizes (3 paint-driven layers)
# Smaller than grass — paved stones read tighter than grass at world scale
BRICK_TILE_SIZE = 3000.0    # rectangular bricks read mid-tight
COBBLE_TILE_SIZE = 2500.0   # rounded cobbles
PEBBLE_TILE_SIZE = 2000.0   # smallest, scattered pebbles read tightest
...
# ============================================================
# SECTION 4.5 (v18 NEW): Paint-driven Pavement Layers
# ============================================================
# Three Landscape paint layers (Brick, Cobble, Pebble) each drive a
# dedicated pavement texture. Painted areas override grass_macro;
# unpainted areas fall through to grass_macro unchanged.
#
# Painter workflow:
#   1. Apply this material (or an MI of it) to the Landscape
#   2. Shift+2 -> Landscape Mode -> Paint tab
#   3. Layer infos appear: Brick / Cobble / Pebble (also GrassDense / FlowerPatch / Debris from Section 11)
#   4. Click + next to each -> Create Layer Info -> save
#   5. Paint plaza as "Brick", streets as "Cobble", side paths as "Pebble"
# Result: each district shows its own pavement texture with smooth blends.

unreal.log("  Creating paint-driven pavement layers (Brick/Cobble/Pebble)...")

# UV dividers — each pavement gets its own irrational tile size
brick_uv = make_uv_div_param("BrickTileSize", BRICK_TILE_SIZE, 800)
cobble_pave_uv = make_uv_div_param("CobbleTileSize", COBBLE_TILE_SIZE, 900)
pebble_uv = make_uv_div_param("PebbleTileSize", PEBBLE_TILE_SIZE, 1000)

# Pavement texture parameters (3 NEW slots)
tex_brick = mel.create_material_expression(
    mat, unreal.MaterialExpressionTextureSampleParameter2D, -1000, 800)
tex_brick.set_editor_property("parameter_name", "BrickTexture")
mel.connect_material_expressions(brick_uv, "", tex_brick, "UVs")

tex_cobble_pave = mel.create_material_expression(
    mat, unreal.MaterialExpressionTextureSampleParameter2D, -1000, 900)
tex_cobble_pave.set_editor_property("parameter_name", "CobbleTexture")
mel.connect_material_expressions(cobble_pave_uv, "", tex_cobble_pave, "UVs")

tex_pebble = mel.create_material_expression(
    mat, unreal.MaterialExpressionTextureSampleParameter2D, -1000, 1000)
tex_pebble.set_editor_property("parameter_name", "PebbleTexture")
mel.connect_material_expressions(pebble_uv, "", tex_pebble, "UVs")

# Try to assign default pavement textures
pave_tex_candidates = {
    "BrickTexture": [
        "/Game/SabriMMO/Textures/Environment/Ground/T_Brick_RO",
        "/Game/SabriMMO/Textures/Environment/T_Brick_RO",
    ],
    "CobbleTexture": [
        "/Game/SabriMMO/Textures/Environment/Ground_Seamless/T_Cobble_RO_2k",
        "/Game/SabriMMO/Textures/Environment/Ground_2K/T_Cobble_RO_2k",
        "/Game/SabriMMO/Textures/Environment/Ground/T_Cobble_RO",
    ],
    "PebbleTexture": [
        "/Game/SabriMMO/Textures/Environment/Ground/T_Pebble_RO",
        "/Game/SabriMMO/Textures/Environment/T_Pebble_RO",
    ],
}
pave_tex_nodes = {
    "BrickTexture": tex_brick,
    "CobbleTexture": tex_cobble_pave,
    "PebbleTexture": tex_pebble,
}
for param_name, node in pave_tex_nodes.items():
    assigned = False
    for candidate in pave_tex_candidates[param_name]:
        tex = try_load_texture(candidate)
        if tex:
            node.set_editor_property("texture", tex)
    ...
    if not assigned:
        unreal.log_warning(
            f"    {param_name}: no texture found yet. "
            "Generate via _tools/generate_pryth_pavements.py and import into UE5, "
            "OR assign manually in the Material Instance."
        )

# Layer weights via LandscapeLayerWeight (Base=0, Layer=1 -> output is the weight 0..1).
# CRITICAL: must use LandscapeLayerWeight (not LandscapeLayerSample) so UE5's
# Landscape paint UI registers and exposes these as paintable layer infos.
# v17's GrassDense/FlowerPatch/Debris use the same pattern.

# Brick weight extractor
weight_brick = mel.create_material_expression(
    mat, unreal.MaterialExpressionLandscapeLayerWeight, -800, 1100)
weight_brick.set_editor_property("parameter_name", "Brick")
weight_brick.set_editor_property("preview_weight", 0.0)
brick_base_const = mel.create_material_expression(mat, unreal.MaterialExpressionConstant, -950, 1080)
brick_base_const.set_editor_property("r", 0.0)
brick_layer_const = mel.create_material_expression(mat, unreal.MaterialExpressionConstant, -950, 1120)
brick_layer_const.set_editor_property("r", 1.0)
mel.connect_material_expressions(brick_base_const, "", weight_brick, "Base")
mel.connect_material_expressions(brick_layer_const, "", weight_brick, "Layer")

# Cobble weight extractor
weight_cobble = mel.create_material_expression(
    mat, unreal.MaterialExpressionLandscapeLayerWeight, -800, 1180)
weight_cobble.set_editor_property("parameter_name", "Cobble")
weight_cobble.set_editor_property("preview_weight", 0.0)
cobble_base_const = mel.create_material_expression(mat, unreal.MaterialExpressionConstant, -950, 1160)
cobble_base_const.set_editor_property("r", 0.0)
cobble_layer_const = mel.create_material_expression(mat, unreal.MaterialExpressionConstant, -950, 1200)
cobble_layer_const.set_editor_property("r", 1.0)
mel.connect_material_expressions(cobble_base_const, "", weight_cobble, "Base")
mel.connect_material_expressions(cobble_layer_const, "", weight_cobble, "Layer")

# Pebble weight extractor
weight_pebble = mel.create_material_expression(
    mat, unreal.MaterialExpressionLandscapeLayerWeight, -800, 1260)
weight_pebble.set_editor_property("parameter_name", "Pebble")
weight_pebble.set_editor_property("preview_weight", 0.0)
pebble_base_const = mel.create_material_expression(mat, unreal.MaterialExpressionConstant, -950, 1240)
pebble_base_const.set_editor_property("r", 0.0)
pebble_layer_const = mel.create_material_expression(mat, unreal.MaterialExpressionConstant, -950, 1280)
pebble_layer_const.set_editor_property("r", 1.0)
mel.connect_material_expressions(pebble_base_const, "", weight_pebble, "Base")
mel.connect_material_expressions(pebble_layer_const, "", weight_pebble, "Layer")

# Weighted color contributions: tex × weight
brick_weighted = mel.create_material_expression(mat, unreal.MaterialExpressionMultiply, -600, 800)
mel.connect_material_expressions(tex_brick, "RGB", brick_weighted, "A")
mel.connect_material_expressions(weight_brick, "", brick_weighted, "B")

cobble_weighted = mel.create_material_expression(mat, unreal.MaterialExpressionMultiply, -600, 900)
mel.connect_material_expressions(tex_cobble_pave, "RGB", cobble_weighted, "A")
mel.connect_material_expressions(weight_cobble, "", cobble_weighted, "B")

pebble_weighted = mel.create_material_expression(mat, unreal.MaterialExpressionMultiply, -600, 1000)
mel.connect_material_expressions(tex_pebble, "RGB", pebble_weighted, "A")
mel.connect_material_expressions(weight_pebble, "", pebble_weighted, "B")

# Sum the weighted colors — additive blend
brick_plus_cobble = mel.create_material_expression(mat, unreal.MaterialExpressionAdd, -400, 850)
mel.connect_material_expressions(brick_weighted, "", brick_plus_cobble, "A")
mel.connect_material_expressions(cobble_weighted, "", brick_plus_cobble, "B")

paint_color = mel.create_material_expression(mat, unreal.MaterialExpressionAdd, -250, 900)
mel.connect_material_expressions(brick_plus_cobble, "", paint_color, "A")
mel.connect_material_expressions(pebble_weighted, "", paint_color, "B")

# Total paint weight = clamp(weight_brick + weight_cobble + weight_pebble, 0, 1)
weight_sum_bc = mel.create_material_expression(mat, unreal.MaterialExpressionAdd, -600, 1100)
mel.connect_material_expressions(weight_brick, "", weight_sum_bc, "A")
mel.connect_material_expressions(weight_cobble, "", weight_sum_bc, "B")

total_paint_unclamped = mel.create_material_expression(mat, unreal.MaterialExpressionAdd, -500, 1180)
mel.connect_material_expressions(weight_sum_bc, "", total_paint_unclamped, "A")
mel.connect_material_expressions(weight_pebble, "", total_paint_unclamped, "B")

total_paint_clamp = mel.create_material_expression(mat, unreal.MaterialExpressionClamp, -350, 1180)
mel.connect_material_expressions(total_paint_unclamped, "", total_paint_clamp, "")

# base_surface = Lerp(grass_macro, paint_color, total_paint_clamp)
# When unpainted (total_paint=0): base_surface = grass_macro (v17 baseline)
# When fully painted (total_paint=1): base_surface = paint_color
base_surface = mel.create_material_expression(mat, unreal.MaterialExpressionLinearInterpolate, -150, 0)
mel.connect_material_expressions(grass_macro, "", base_surface, "A")
mel.connect_material_expressions(paint_color, "", base_surface, "B")
mel.connect_material_expressions(total_paint_clamp, "", base_surface, "Alpha")
...
# Lerp: BaseSurface (paint-blended grass_macro) ↔ Dirt based on dirt mask
# v18: input is base_surface (Section 4.5 output) instead of grass_macro,
# so paint layers are visible through dirt patches too. Unpainted areas
# behave identically to v17 since base_surface == grass_macro when total_paint=0.
grass_dirt_lerp = mel.create_material_expression(mat, unreal.MaterialExpressionLinearInterpolate, -100, -50)
mel.connect_material_expressions(base_surface, "", grass_dirt_lerp, "A")
...
with open(tracking_path, "a") as f:
    ...
    f.write(f"{roughness_const.get_editor_property('default_value'):.2f} | ")
    import datetime
...
unreal.log("=" * 60)
unreal.log("")
unreal.log("=== 29 PARAMETERS (all MI-overridable) ===")
unreal.log("")
unreal.log("Textures (12 — 3 NEW for v18):")
...
unreal.log("  [NEW v18] BrickTexture, CobbleTexture, PebbleTexture")
unreal.log("")
unreal.log("Color Processing (4):")
unreal.log("  WarmthTint (Vector) | SaturationMult | BrightnessOffset | ContrastBoost")
unreal.log("")
unreal.log("Blending (4):")
unreal.log("  GrassVariantBalance | GrassNoiseScale | MacroNoiseScale | DirtOnSlopes")
unreal.log("")
unreal.log("Slope (4):")
unreal.log("  SlopeThreshold | SlopeTransitionWidth | SlopeNoiseAmount | SlopeNoiseFreq")
unreal.log("")
unreal.log("Surface (5):")
unreal.log("  DirtAmount | Roughness | NormalStrength | AOStrength | UVDistortStrength")
unreal.log("")
unreal.log("Tile Sizes (7 — 3 NEW for v18):")
unreal.log("  GrassWarmTileSize, GrassCoolTileSize, DirtTileSize, RockTileSize")
unreal.log("  [NEW v18] BrickTileSize, CobbleTileSize, PebbleTileSize")
unreal.log("")
unreal.log("=== 6 PAINT LAYERS (Landscape Mode -> Paint tab) ===")
unreal.log("")
unreal.log("v18 NEW — drive base color blending:")
unreal.log("  Brick   -> samples BrickTexture")
unreal.log("  Cobble  -> samples CobbleTexture (default if no other layer painted)")
unreal.log("  Pebble  -> samples PebbleTexture")
unreal.log("")
unreal.log("Existing v17 — drive Grass Output sprite scatter:")
unreal.log("  GrassDense   -> grass varieties")
unreal.log("  FlowerPatch  -> flower varieties")
unreal.log("  Debris       -> rock/leaf/twig varieties")
unreal.log("")
unreal.log("Features:")
...
unreal.log("  - [v18] Paint-driven 3-layer pavement blending")
...
unreal.log("  2. Assign textures if not auto-detected (especially BrickTexture, PebbleTexture)")
unreal.log("  3. Create Landscape in level, assign this material (or an MI of it)")
unreal.log("  4. Shift+2 -> Landscape Mode -> Paint tab")
unreal.log("  5. Create Layer Info for each: Brick, Cobble, Pebble (and Grass*/Flower*/Debris)")
unreal.log("  6. Paint plaza as Brick, streets as Cobble, side paths as Pebble")
unreal.log("")
unreal.log("MANUAL STEP STILL REQUIRED:")
unreal.log("  Add a LandscapeGrassOutput node manually in Material Editor")
unreal.log("  (Python cannot configure Grass Output — would cause compile failure)")
unreal.log("  Connect grass_dense_final / flower_final / debris_final to grass varieties")
