"""RECOVERY part 1 of 7: re-import priest_f body atlases from PNG.

Run from UE5 Python console:
    py "C:/Sabri_MMO/client/SabriMMO/Scripts/Environment/migrate_recover_01_priest.py"

After it finishes, wait for "Compiling Textures: 0" before running part 2.
"""
import unreal
import os
import glob
...
DISK_DIR = os.path.join(ATLAS_ROOT, "Body/priest_f")
GAME_DIR = f"{GAME_ROOT}/Body/priest_f"
...
def main():
    ...
    unreal.log(f"=== RECOVERY 1/7: priest_f body ({len(pngs)} atlases) ===")
    counts = {"recovered": 0, "import_failed": 0, "conflicts_deleted": 0}
    start = time.time()
    for i, png in enumerate(pngs, 1):
        name = os.path.splitext(os.path.basename(png))[0]
        result, n_conflicts = recover_one(png, GAME_DIR, f"{GAME_DIR}/{name}")
    ...
    unreal.log(f"Wait for 'Compiling Textures: 0' before running part 2.")
